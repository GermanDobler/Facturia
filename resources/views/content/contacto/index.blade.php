...
@section('content')
    ...
    <form action="{{ route('contacto.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <h4 class="py-3 mb-4">
            <span class="text-muted fw-light">Pagina /</span> Contacto <button type="submit"
                    ...
                    <div class="row mb-3">
                        <div class="col-4">
                            ...
                            <input type="file" class="form-control" id="portada" name="portada"
                                {{ empty($contacto->portada) ? 'required' : '' }}>
                        </div>
                        <div class="col-4">
                            <label class="col-form-label" for="titulo">Titulo</label>
                            <input type="text" class="form-control" id="titulo" name="titulo"
                                placeholder="(opcional)" value="{{ old('titulo', $contacto->titulo ?? '') }}">
                        </div>
                        <div class="col-4">
                            <label class="col-form-label" for="subtitulo">Subtitulo</label>
                            <input type="text" class="form-control" id="subtitulo" name="subtitulo"
                                placeholder="(opcional)" value="{{ old('subtitulo', $contacto->subtitulo ?? '') }}">
                        </div>
                    </div>
                    <div id="image-portada" class="row">
                        @if (!empty($contacto->portada))
                            <!-- Mostrar imagen previa de la portada -->
                            <div class="row">
                                <img src="{{ asset(env('IMAGE_PATH') . basename($contacto->portada)) }}"
                                    class="card-img-top" alt="Image preview"
                                    style="object-fit: cover; height: 520px; width: 100%;">
                            </div>
                        @endif
                    </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <input type="hidden" id="descripcion" name="descripcion"
                        value="{{ old('descripcion', $contacto->descripcion ?? '') }}">
                </div>
                {{-- <div class="row mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var descripcion = @json($contacto->descripcion);
        if (descripcion) {
            quill.root.innerHTML = descripcion;
        }

        quill.on('text-change', function() {
            // Actualizar el input oculto con el contenido del editor
