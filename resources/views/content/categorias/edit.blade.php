    ...
    <div class="card">
        <form action="{{ isset($categoria) ? route('categorias.update', $categoria) : route('categorias.store') }}"
            ...
            <div class="card-body">
                ...
                <!-- Es subcategoría -->
                @if ($categoria->subcategorias->isEmpty() ?? true)
                    <div class="form-check">
                        <input type="checkbox" name="es_subcategoria" id="es_subcategoria" class="form-check-input"
                            value="1"
                            {{ old('es_subcategoria', $categoria->es_subcategoria ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="es_subcategoria">¿Es subcategoría?</label>
                    </div>
                @else
                    <div class="alert alert-warning" role="alert">
                        Esta categoría no puede ser marcada como subcategoría porque ya tiene subcategorías asociadas.
                    </div>
                @endif

                <!-- Selección de categoría padre -->
                <div class="form-group" id="categoria_padre_select"
                    style="display: {{ old('es_subcategoria', $categoria->es_subcategoria ?? false) ? 'block' : 'none' }};">
                    ...
                    <select name="categoria_padre_id" id="categoria_padre_id" class="form-control">
                        ...
                        @foreach ($categorias as $cat)
                            @if (!isset($categoria) || $categoria->id !== $cat->id)
                                <option value="{{ $cat->id }}"
                                    {{ old('categoria_padre_id', $categoria->categoria_padre_id ?? null) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->nombre }}
                                </option>
                            @endif
                        @endforeach
